...
const Proyectos = () => {
  ...
  useEffect(() => {
    const fetchProyectos = async () => {
      try {
        const res = await fetch('http://localhost:3002/proyectos/proyectos', {
          method: 'POST',
  ...
  useEffect(() => {
    const fetchUsuarios = async () => {
      try {
        const res = await fetch('http://localhost:3002/usuarios/usuarios', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
const TerminarProyecto = async (id: number) => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        const res = await fetch('http://localhost:3002/proyectos/terminarProyecto', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  const eliminarProyecto = async (id: number) => {
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        const res = await fetch('http://localhost:3002/proyectos/cancelarProyecto', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ Luser, Lpass, id }),
        });
  ...
  const handleGuardar = async () => {
    ...
    const url = modoEdicion
      ? 'http://localhost:3002/proyectos/editarProyecto'
      : 'http://localhost:3002/proyectos/crearP';
