...
const Usuarios = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsuarios = async () => {
      ...
      setRolUsuario(rol);
      try {
        const response = await fetch('http://localhost:3002/usuarios/usuarios', {
          method: 'POST',
  ...
  const handleEliminar = async (id_usuario: string) => {
    ...
    if (confirmacion.isConfirmed) {
      try {
        const response = await fetch('http://localhost:3002/usuarios/editarUsuario', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  imagen: usuarioEditando.imagen, 
    ...
    const url = modoEdicion
      ? 'http://localhost:3002/usuarios/editarUsuario'
      : 'http://localhost:3002/usuarios/crearUsuario';
